...
var ObjFuncers = []shapes.Funcer{
	...
	shapes.Funcer{InputType: types.Obj{
	...
	shapes.Funcer{InputType: types.Obj{
		Props: map[string]types.Type{},
		Rest:  types.NewVar("A", types.Any{}),
	}, Name: "items", Params: nil, OutputType: types.NewArr(
	...
	shapes.Funcer{InputType: types.Obj{
		Props: map[string]types.Type{},
		Rest:  types.NewVar("A", types.Any{}),
	}, Name: "props", Params: nil, OutputType: types.NewArr(types.Str{}), Kernel: func(inputState states.State, args []states.Action, bindings map[string]types.Type, pos lexer.Position) *states.Thunk {
		inputValue := inputState.Value.(states.ObjValue)
		c := make(chan *states.Thunk)
		go func() {
	...
	shapes.Funcer{InputType: types.Obj{
		Props: map[string]types.Type{},
		Rest:  types.NewVar("A", types.Any{}),
	}, Name: "values", Params: nil, OutputType: types.NewArr(types.NewVar("A", types.Any{})), Kernel: func(inputState states.State, args []states.Action, bindings map[string]types.Type, pos lexer.Position) *states.Thunk {
		inputValue := inputState.Value.(states.ObjValue)
		c := make(chan *states.Thunk)
		go func() {
